...
const GAME_CONFIG = {
  ...
  rewards: {
    coinflip: Math.max(50, Number(CONFIG.GAME_REWARDS?.COINFLIP ?? 50)),
    dice: Math.max(50, Number(CONFIG.GAME_REWARDS?.DICE ?? 50)),
    number: Math.max(50, Number(CONFIG.GAME_REWARDS?.NUMBER ?? 50)),
    scratch: Math.max(50, Number(CONFIG.GAME_REWARDS?.SCRATCH ?? 50)),
    wheel: Math.max(50, Number(CONFIG.GAME_REWARDS?.WHEEL ?? 50)),
    mystery: Math.max(50, Number(CONFIG.GAME_REWARDS?.MYSTERY ?? 100))
  }
};
...
async function playGame(req, res) {
  ...
  // Games are free-to-play. They never deduct a user's earned balance.
  // The server decides the result and reward.
  const bet = 0;
  const ref = db.collection("users").doc(uid);
  let result = null;
  let levelUp = false;
  ...
  await db.runTransaction(async tx => {
    ...
    let outcome = "";
    let mysteryReward = 50;
    const requestedChoice = String(req.body?.choice || "").toLowerCase();
    const baseGameReward = Math.max(50, safeInt(GAME_CONFIG.rewards[game], 50));

    if (game === "coinflip") {
      const side = Math.random() < 0.5 ? "heads" : "tails";
      won = requestedChoice === side;
      multiplier = won ? 1 : 0;
      outcome = `The coin landed on ${side === "heads" ? "HEADS" : "TAILS"}.`;
    } else if (game === "dice") {
      const roll = randomInt(1, 6);
      const pick = requestedChoice === "low" ? "low" : "high";
      won = pick === "high" ? roll >= 4 : roll <= 3;
      multiplier = won ? 1 : 0;
      outcome = `The dice rolled ${roll}. You chose ${pick === "high" ? "HIGH (4–6)" : "LOW (1–3)"}.`;
    } else if (game === "number") {
      const target = clamp(safeInt(requestedChoice, 3), 1, 5);
      const drawn = randomInt(1, 5);
      won = target === drawn;
      multiplier = won ? 1 : 0;
      outcome = `Lucky number: ${drawn}. You picked ${target}.`;
    } else if (game === "scratch") {
      const symbols = ["🍀", "⭐", "💎", "🎁", "💰"];
      const symbol = symbols[randomInt(0, symbols.length - 1)];
      won = Math.random() < 0.45;
      multiplier = won ? 1 : 0;
      outcome = won ? `Your card revealed ${symbol} — WIN!` : `Your card revealed ${symbol} — no prize this time.`;
    } else if (game === "wheel") {
      const slices = ["50 PTS", "TRY AGAIN", "50 PTS", "BONUS", "50 PTS", "TRY AGAIN"];
      const slice = slices[randomInt(0, slices.length - 1)];
      won = slice !== "TRY AGAIN";
      multiplier = won ? 1 : 0;
      outcome = `The wheel stopped at ${slice}.`;
    } else if (game === "mystery") {
      mysteryReward = [50, 50, 50, 75, 100][randomInt(0, 4)];
      won = true;
      multiplier = 1;
      outcome = `Mystery reward unlocked: ${mysteryReward} PTS!`;
    }
    display = outcome || "Round complete";
    const reward = won ? (game === "mystery" ? Math.max(50, mysteryReward) : baseGameReward) : 0;
    const net = reward;
...
async function gameStatus(req, res) {
  ...
  return res.status(200).json({
    ...
    cooldownReady: !last || (Date.now() - last >= GAME_CONFIG.cooldownMs),
