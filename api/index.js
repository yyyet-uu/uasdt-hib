...
// Daily Streak Rewards Table (PTS)
...
// Check membership in Telegram channels
...
// Send automated payout proof notification to channel
...
// Execute on-chain BEP20 USDT Transfer
async function sendBscUsdt(recipientAddress, amountUsdt) {
  if (!PAYOUT_PRIVATE_KEY) {
    throw new Error("Payout wallet is not configured on the server.");
  }
...
module.exports = async function handler(req, res) {
  ...
  // Public Endpoint: Ad Unit Crawler Fallback
  if (!tgUser) {
    if (endpoint === "public-info") {
      return res.json({ success: true, name: "USDT Hub", active: true });
    }
  ...
  try {
    ...
    // ----------------------------------------------------
    // 3. CLAIM WELCOME BONUS (0.01 USDT AUTO ON-CHAIN)
    // ----------------------------------------------------
    ...
    // ----------------------------------------------------
    // 5. MINING ADS (MONETAG [25] + HILLTOPADS [15])
    // ----------------------------------------------------
    if (endpoint === "ads") {
      ...
      // Second-tier referral reward (+500 PTS when referral completes 2 ads)
    ...
    if (endpoint === "games") {
      ...
      // Deterministic multiplier calculation matching client formula
    ...
    if (endpoint === "tasks") {
      ...
      if (action === "complete") {
        ...
        // Verify Telegram Chat Membership if chatId is provided
        if (taskData.chatId) {
