...
const GAME_CONFIG = {
  // Two chances for EACH game every UTC day.
  chancesPerGame: Number(CONFIG.GAME_CHANCES_PER_GAME ?? 2),
  totalDailyLimit: 12,
  cooldownMs: Number(CONFIG.GAME_COOLDOWN_MS ?? 3000),
  rewards: {
    coinflip: 50,
    dice: 75,
    number: 100,
...
const GAME_NAMES = new Set([
  "coinflip", "dice", "number", "scratch", "wheel", "mystery"
]);
...
function getGameCounts(user) {
  const stored = user.gamePlays && typeof user.gamePlays === "object" ? user.gamePlays : {};
  const fresh = user.gameDate === today();
  const counts = {};
  for (const name of GAME_NAMES) counts[name] = fresh ? safeInt(stored[name], 0) : 0;
  return counts;
}

function totalGamePlays(counts) {
  return Object.values(counts).reduce((sum, value) => sum + safeInt(value, 0), 0);
}
...
async function playGame(req, res) {
  ...
  const requestId = String(req.body?.requestId || "").slice(0, 100);
  ...
  try {
    await db.runTransaction(async tx => {
      ...
      // Idempotency: if the same request was already processed, return the
      // saved result instead of awarding another reward.
      if (requestId && u.lastGameRequestId === requestId && u.lastGameResult) {
        result = u.lastGameResult;
        return;
      }
      ...
      const counts = getGameCounts(u);
      const total = totalGamePlays(counts);
      if (counts[game] >= GAME_CONFIG.chancesPerGame) throw new Error("GAME_CHANCES_USED");
      if (total >= GAME_CONFIG.totalDailyLimit) throw new Error("GAME_DAILY_LIMIT");
      ...
      if (game === "coinflip") {
        if (!["heads", "tails"].includes(choice)) throw new Error("MAKE_A_CHOICE");
        const landed = Math.random() < 0.5 ? "heads" : "tails";
        won = choice === landed;
        reveal = { result: landed, choice };
        outcome = won ? `You called ${landed.toUpperCase()} — correct!` : `It landed ${landed.toUpperCase()}.`;
      } else if (game === "dice") {
        if (!["high", "low"].includes(choice)) throw new Error("MAKE_A_CHOICE");
        const roll = randomInt(1, 6);
        won = choice === "high" ? roll >= 4 : roll <= 3;
        reveal = { roll, choice };
        outcome = `The dice landed on ${roll}. ${won ? "Your call was correct!" : "Your call missed."}`;
      } else if (game === "number") {
        const target = clamp(safeInt(choice, 0), 1, 5);
        if (!target) throw new Error("MAKE_A_CHOICE");
        const lucky = randomInt(1, 5);
        won = target === lucky;
        reveal = { lucky, choice: target };
        outcome = won ? `Bullseye! ${lucky} was the lucky number.` : `The lucky number was ${lucky}.`;
      } else if (game === "scratch") {
        const card = clamp(safeInt(choice, 0), 1, 3);
        if (!card) throw new Error("MAKE_A_CHOICE");
        const symbols = Math.random() < 0.55 ? ["💎", "💎", "💎"] : ["🍀", "⭐", "🎯"];
        won = symbols[0] === symbols[1] && symbols[1] === symbols[2];
        reveal = { card, symbols };
        outcome = won ? "Three matching symbols — winning card!" : "No match this time.";
      } else if (game === "wheel") {
        const segments = [
        ...
        won = segment.win;
        reveal = { segment: segment.label, index: segments.indexOf(segment) };
        reward = segment.reward;
        outcome = won ? `The wheel stopped on ${segment.label}.` : "The wheel stopped on MISS.";
      } else if (game === "mystery") {
        const box = clamp(safeInt(choice, 0), 1, 3);
        if (!box) throw new Error("MAKE_A_CHOICE");
        const roll = Math.random();
        rarity = roll > 0.92 ? "ULTRA RARE" : roll > 0.72 ? "EPIC" : roll > 0.45 ? "RARE" : "COMMON";
        reward = rarity === "ULTRA RARE" ? 300 : rarity === "EPIC" ? 200 : rarity === "RARE" ? 150 : 50;
        won = true;
        reveal = { box };
        outcome = `${rarity} reward discovered!`;
      }

      if (game !== "wheel" && game !== "mystery") {
        reward = won ? Number(GAME_CONFIG.rewards[game] || 50) : 0;
      }

      const xpGain = won ? 25 : 10;
      const nextCounts = { ...counts, [game]: counts[game] + 1 };
      const gamesToday = totalGamePlays(nextCounts);
      const lastGameResult = {
        game, choice, won, reward, net: reward, outcome, display: outcome, reveal, rarity,
        xpGain, gamesToday, gamesRemaining: Math.max(0, GAME_CONFIG.totalDailyLimit - gamesToday),
        gameRemaining: Math.max(0, GAME_CONFIG.chancesPerGame - nextCounts[game]),
        requestId
      };
      ...
      const update = {
        ...
        gameDate: today(),
        gamesToday,
        gamePlays: nextCounts,
        lastGameAt: now,
        updatedAt: FieldValue.serverTimestamp()
      };
      if (requestId) {
        update.lastGameRequestId = requestId;
        update.lastGameResult = lastGameResult;
      }
  ...
  // Achievement processing must never make an already-completed game fail.
  let newAchievements = [];
  try {
    newAchievements = await applyAchievements(uid);
  } catch (achievementError) {
    console.error("GAME ACHIEVEMENT UPDATE SKIPPED:", achievementError);
  }
...
async function gameStatus(req, res) {
  ...
  const u = snap.data();
  const counts = getGameCounts(u);
  const played = totalGamePlays(counts);
  const remaining = {};
  for (const game of GAME_NAMES) remaining[game] = Math.max(0, GAME_CONFIG.chancesPerGame - counts[game]);
  const last = Number(u.lastGameAt || 0);
  return res.status(200).json({
    success: true,
    dailyLimit: GAME_CONFIG.totalDailyLimit,
    gamesToday: played,
    gamesRemaining: Math.max(0, GAME_CONFIG.totalDailyLimit - played),
    chancesPerGame: GAME_CONFIG.chancesPerGame,
    perGame: counts,
    remainingByGame: remaining,
