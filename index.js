...
async function playGame(req, res) {
  ...
  await db.runTransaction(async tx => {
    ...
    let won = false;
    let multiplier = 0;
    let display = "Try again!";

    const baseGameReward = Math.max(1, safeInt(GAME_CONFIG.rewards[game], 5));

    if (game === "coinflip") {
      won = Math.random() >= 0.5;
      multiplier = won ? 1 : 0;
      display = won ? "Heads — reward unlocked!" : "Tails — try again";
    } else if (game === "dice") {
      ...
      const target = clamp(safeInt(req.body?.number, 3), 1, 5);
      const drawn = randomInt(1, 5);
      won = target === drawn;
      multiplier = won ? 1 : 0;
      display = `Lucky number: ${drawn}`;
    } else if (game === "scratch") {
      won = Math.random() >= 0.65;
      multiplier = won ? 1 : 0;
      display = won ? "Scratch WIN!" : "No prize this time";
    } else if (game === "wheel") {
      const roll = Math.random();
      won = roll >= 0.45;
      multiplier = won ? 1 : 0;
      display = won ? "Wheel reward!" : "Wheel missed";
    } else if (game === "mystery") {
      won = true;
      multiplier = 1;
      display = "Mystery box opened!";
    }

    const reward = won ? baseGameReward : 0;
    const net = reward;
