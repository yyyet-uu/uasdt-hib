...
const GAME_CONFIG = {
  dailyLimit: Number(CONFIG.GAME_DAILY_LIMIT ?? 20),
...
async function playGame(req, res) {
  ...
  try {
    await db.runTransaction(async tx => {
      ...
      const gamesToday = u.gameDate === today() ? Number(u.gamesToday || 0) : 0;
      ...
      const gameResult = {
        ...
        gamesRemaining: Math.max(0, GAME_CONFIG.dailyLimit - gamesToday - 1),
