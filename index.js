...
async function referral(
  ...
  if (
    ...
    const enriched = await Promise.all(referrals.map(async (item) => {
      const referredId = String(item.referredUserId || item.referredId || "");
      if (!referredId) return item;
      try {
        ...
        return {
          ...
          lastName: u.lastName || "",
          username: u.username || "",
          referredUserName: u.firstName || u.username || "User"
        };
      } catch (lookupError) {
        console.error("REFERRAL USER LOOKUP ERROR", lookupError);
        return item;
      }
    }));
...
const GAME_CONFIG = {
  ...
  minBet: Number(CONFIG.GAME_MIN_BET ?? 0),
  maxBet: Number(CONFIG.GAME_MAX_BET ?? 0),
  rewards: {
    coinflip: Number(CONFIG.GAME_REWARDS?.COINFLIP ?? 50),
    dice: Number(CONFIG.GAME_REWARDS?.DICE ?? 75),
    number: Number(CONFIG.GAME_REWARDS?.NUMBER ?? 100),
    scratch: Number(CONFIG.GAME_REWARDS?.SCRATCH ?? 75),
    wheel: Number(CONFIG.GAME_REWARDS?.WHEEL ?? 100),
    mystery: Number(CONFIG.GAME_REWARDS?.MYSTERY ?? 150)
  }
};
...
async function dashboard(req, res) {
  ...
  return res.status(200).json({
    success: true,
    today: { date: d, earnings: Number(u.todayEarned || 0), ads: adsToday, tasks: Number(u.todayTasks || 0), referrals: Number(u.todayReferrals || 0) },
    totals: { earned: Number(u.lifetimeEarned || 0), withdrawn: Number(u.totalWithdrawn || 0), ads: Number(u.adsWatched || 0), tasks: Number(u.tasksCompleted || 0), referrals: Number(u.referrals || 0) },
    streak: { day: Number(u.streakDay || 0), lastDate: u.lastStreakDate || null },
    level, rank, vip: { name: vip.name, multiplier: vip.multiplier }, mission,
...
async function leaderboard(req, res) {
  ...
  const field = allowed.has(type) ? ({ earnings: "balance", ads: "adsWatched", tasks: "tasksCompleted", referrals: "referrals" })[type] : "balance";
  const snap = await db.collection("users").orderBy(field, "desc").limit(50).get();
  const rows = snap.docs.map((doc, index) => {
    const u = doc.data();
    return { rank: index + 1, userId: doc.id, firstName: u.firstName || "Explorer", username: u.username || "", value: Number(u[field] || 0), isMe: doc.id === uid, badge: index === 0 ? "👑" : index === 1 ? "🥈" : index === 2 ? "🥉" : "" };
  });
...
async function achievements(req, res) {
  ...
  return res.status(200).json({ success: true, achievements: getAchievementDefinitions().map(a => ({ ...a, unlocked: unlocked.has(a.id), progress: clamp(Number(u[a.field] || 0), 0, a.target), target: a.target })) });
}
...
async function playGame(req, res) {
  ...
  const requestId = String(req.body?.requestId || `${Date.now()}_${Math.random()}`).replace(/[^a-zA-Z0-9_-]/g, "").slice(0, 80);
  const ref = db.collection("users").doc(uid);
  const requestRef = ref.collection("gameRequests").doc(requestId || `${Date.now()}`);
  let result = null;

  try {
    await db.runTransaction(async tx => {
      const cached = await tx.get(requestRef);
      if (cached.exists) {
        result = cached.data()?.result || null;
        return;
      }
      ...
      const gamesToday = u.gameDate === todayKey ? Number(u.gamesToday || 0) : 0;
      const gameCounts = u.gameAttemptsDate === todayKey && u.gameAttempts && typeof u.gameAttempts === "object"
        ? { ...u.gameAttempts }
        : {};
      const playedThisGame = Math.max(0, safeInt(gameCounts[game], 0));
      if (playedThisGame >= GAME_CONFIG.chancesPerGame) throw new Error(`GAME_CHANCES_USED_${game}`);
      if (gamesToday >= GAME_CONFIG.dailyLimit) throw new Error("GAME_DAILY_LIMIT");
      if (last && now - last < GAME_CONFIG.cooldownMs) throw new Error(`GAME_COOLDOWN_${Math.ceil((GAME_CONFIG.cooldownMs - (now - last)) / 1000)}`);

      const base = Math.max(50, safeInt(GAME_CONFIG.rewards[game], 50));
      let won = false;
      let reward = 0;
      let outcome = "Round complete";
      let reveal = {};
      let rarity = "COMMON";

      if (game === "coinflip") {
        const choice = String(req.body?.choice || "heads").toLowerCase() === "tails" ? "tails" : "heads";
        const flip = Math.random() < 0.5 ? "heads" : "tails";
        won = choice === flip;
        reward = won ? base : 0;
        outcome = `The coin landed on ${flip.toUpperCase()}.`;
        reveal = { choice, result: flip };
        rarity = won ? "LUCKY CALL" : "CLOSE CALL";
      } else if (game === "dice") {
        const choice = String(req.body?.choice || "high").toLowerCase() === "low" ? "low" : "high";
        const roll = randomInt(1, 6);
        won = choice === "high" ? roll >= 4 : roll <= 3;
        reward = won ? base : 0;
        outcome = `The dice rolled ${roll}.`;
        reveal = { choice, roll, zone: roll >= 4 ? "HIGH" : "LOW" };
        rarity = won ? "PERFECT ROLL" : "NEXT ROLL";
      } else if (game === "number") {
        const pick = clamp(safeInt(req.body?.choice ?? req.body?.number, 1), 1, 5);
        const lucky = randomInt(1, 5);
        won = pick === lucky;
        reward = won ? base : 0;
        outcome = `Lucky number revealed: ${lucky}.`;
        reveal = { pick, lucky, numbers: [1, 2, 3, 4, 5] };
        rarity = won ? "BULLSEYE" : "NEAR MISS";
      } else if (game === "scratch") {
        const card = clamp(safeInt(req.body?.choice, 1), 1, 3);
        const roll = Math.random();
        won = roll < 0.58;
        reward = won ? base : 0;
        const symbols = won ? ["💎", "💎", "💎"] : ["🍀", "🪙", "⭐"];
        outcome = won ? "Three matching symbols! Your card is a winner." : "No match this time. Your next card is waiting.";
        reveal = { card, symbols };
        rarity = won ? "GOLD CARD" : "STANDARD CARD";
      } else if (game === "wheel") {
        const segments = ["50 PTS", "TRY AGAIN", "75 PTS", "LUCKY", "100 PTS", "BONUS", "150 PTS", "TRY AGAIN"];
        const index = randomInt(0, segments.length - 1);
        won = !segments[index].includes("TRY AGAIN");
        reward = won ? Math.max(base, index === 6 ? 150 : index === 4 ? 100 : index === 2 ? 75 : 50) : 0;
        outcome = `The wheel stopped on ${segments[index]}.`;
        reveal = { index, segment: segments[index], segments };
        rarity = reward >= 150 ? "JACKPOT" : won ? "LUCKY SPIN" : "MISSED SPIN";
      } else if (game === "mystery") {
        const box = clamp(safeInt(req.body?.choice, 2), 1, 3);
        const roll = Math.random();
        won = true;
        reward = roll < 0.08 ? 300 : roll < 0.22 ? 200 : roll < 0.48 ? 150 : 50;
        outcome = `Box ${box} opened and revealed ${reward} PTS.`;
        reveal = { box, reward, boxes: [1, 2, 3] };
        rarity = reward >= 300 ? "ULTRA RARE" : reward >= 200 ? "EPIC" : reward >= 150 ? "RARE" : "COMMON";
      }

      const xpGain = won ? 35 : 10;
      const nextXp = Number(u.xp || 0) + xpGain;
      const oldLevel = getLevelInfo(u.xp || 0).level;
      const newLevel = getLevelInfo(nextXp).level;
      const levelUp = newLevel > oldLevel;
      const net = Math.max(0, reward);
      gameCounts[game] = playedThisGame + 1;
      const gameRemaining = Math.max(0, GAME_CONFIG.chancesPerGame - gameCounts[game]);
      const totalRemaining = Math.max(0, GAME_CONFIG.dailyLimit - gamesToday - 1);
      const gameResult = {
        game, won, reward: net, net, outcome, display: outcome, reveal, rarity,
        gamesToday: gamesToday + 1,
        gamesRemaining: gameRemaining,
        gameChancesUsed: gameCounts[game],
        gameChancesRemaining: gameRemaining,
        totalGamesRemaining: totalRemaining,
        xpGain, level: newLevel, levelUp,
        dailyLimit: GAME_CONFIG.dailyLimit,
        chancesPerGame: GAME_CONFIG.chancesPerGame
      };
      tx.update(ref, {
        balance: FieldValue.increment(net),
        lifetimeEarned: FieldValue.increment(net),
        todayEarned: FieldValue.increment(net),
        xp: FieldValue.increment(xpGain),
        gamesPlayed: FieldValue.increment(1),
        gamesWon: FieldValue.increment(won ? 1 : 0),
        gameDate: todayKey,
        gamesToday: gamesToday + 1,
        gameAttemptsDate: todayKey,
        gameAttempts: gameCounts,
        lastGameAt: now,
        updatedAt: FieldValue.serverTimestamp()
      });
      tx.create(requestRef, { result: gameResult, createdAt: FieldValue.serverTimestamp() });
      result = gameResult;
    });
  } catch (error) {
  ...
  let newAchievements = [];
  try {
    newAchievements = await applyAchievements(uid);
  } catch (achievementError) {
    console.error("GAME ACHIEVEMENT ERROR", achievementError);
  }

  const updated = await ref.get();
  return res.status(200).json({ success: true, result, user: publicUser(updated.data()), achievements: newAchievements });
}
...
async function gameStatus(req, res) {
  ...
  const attempts = u.gameAttemptsDate === d && u.gameAttempts && typeof u.gameAttempts === "object" ? u.gameAttempts : {};
  const perGame = {};
  for (const game of GAME_NAMES) {
    const used = Math.max(0, safeInt(attempts[game], 0));
    perGame[game] = { used, remaining: Math.max(0, GAME_CONFIG.chancesPerGame - used), limit: GAME_CONFIG.chancesPerGame };
  }
  const last = Number(u.lastGameAt || 0);
  return res.status(200).json({
    success: true,
...
/* =========================================================
   AD SESSION / RETENTION HELPERS
   These values are calculated server-side and never trusted
   from the browser.
========================================================= */

function calculateAdBonus(user, baseReward, now = new Date()) {
  const d = today();
  const firstAd = user.adDate !== d || Number(user.adsWatchedToday || 0) === 0;
  const previousCombo = user.adComboDate === d ? Number(user.adCombo || 0) : 0;
  const combo = previousCombo + 1;
  const comboBonus = combo > 0 && combo % 5 === 0 ? Math.round(baseReward * 0.5) : 0;
  const firstBonus = firstAd ? Math.max(0, safeInt(CONFIG.FIRST_AD_BONUS ?? 25)) : 0;
  const weekendBonus = [0, 6].includes(now.getUTCDay()) ? Math.max(0, safeInt(CONFIG.WEEKEND_BONUS ?? 0)) : 0;
  const doubleHours = Array.isArray(CONFIG.DOUBLE_POINTS_HOURS) ? CONFIG.DOUBLE_POINTS_HOURS : [];
  const hour = now.getUTCHours();
  const doubleHour = doubleHours.includes(hour);
  const multiplier = doubleHour ? 2 : 1;
  return {
    firstAd,
    combo,
    comboBonus,
    ...
    doubleHour,
    multiplier,
    totalBonus: comboBonus + firstBonus + weekendBonus
  };
}
