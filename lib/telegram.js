 ...
 */
export async function broadcastPaymentProof(
  proof
) {
  const channel =
  ...
  const text = [
    ...
    ``,
    `👤 <b>User:</b> <code>${String(
      proof?.userId || ""
    ).slice(0, 4)}***</code>`,
    `💰 <b>Amount:</b> <code>${proof?.amountUSDT || 0} USDT</code>`,
    ``,
    `🔗 <b>Transaction:</b>`,
    `<a href="https://bscscan.com/tx/${txHash}">${txHash.slice(
      0,
      14
    )}...</a>`,
    ``,
...
export async function notifyNewReferral(
  ...
  return await sendMessage(
    ...
    [
      ...
      `${escapeHTML(
        name
      )} just joined using your referral link.`,
      ``,
